...
from data_simulacion import Equipos, Equipos_torneo_estatico
import pandas as pd
...
fecha_inicial = int(input("ingrese el numero de la fecha desde la cual desea comenzar a optimizar inclusive: "))
...
class Torneo:
    ...
    def avanzar_el_calendario(self):
        n = int(input("ingrese el numero de la fecha hasta la cual desea avanzar inclusive: "))
        print("")
...
#aqui borramos la info de partidos simulados para actualizarla luego con partidos reales
    ...
    def generar_datos_visualizacion(self):
...
while boleano == False:
    ...
    imput = int(input("Introduzca el numero: "))
    ...
    if imput == 1:
        ...
        fecha_inicial = int(input("ingrese el numero de la fecha desde la cual desea optimizar inclusive: "))
        file = open("fecha_actual.txt", "w") #actualizar para que este en la otra carpeta
        file.write(str(fecha_inicial))
        file.close()
        #AQUI importar fecha_inicial a params.py y correr model.py ===>
        ...
        atractivo_real = torneo_2019.generar_datos_visualizacion()
        print(atractivo_real)
